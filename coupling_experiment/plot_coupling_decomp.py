...
def decompose(th, ve, thb):
    ...
    return B * 100, pV * (thV - B) * 100, (1 - pV) * (thN - B) * 100
...
for sub, tag, lbl in [(None, "full256", "256 prompts"), (OBF, "obf22", "22 obfuscated prompts")]:
    ...
    for cname, clab in CONDS:
        for m, mlab in MODELS:
            ...
            thb = baseline(m, sub)
            B, ver, sil = decompose(th, ve, thb); eV, eS = boot(th, ve, thb); bvals[m] = B
            ...
            top = ver + sil + eS
            ax.text(pos, top + 1.5, f"VEA {ver:+.0f}", ha="center", fontsize=7.5, color=VEA_C, weight="bold")
            ax.text(pos, top + 5.0, f"Latent {sil:+.1f}", ha="center", fontsize=7.5, color=LAT_C, weight="bold")
            xs.append(pos); xt.append(f"{clab}\n{mlab}"); pos += 0.72
        pos += 0.45
    ax.axhline(0, color="#888", lw=1)
    ax.set_ylim(0, ax.get_ylim()[1] * 1.18)   # headroom so VEA/Latent labels clear the title
    ax.set_xticks(xs); ax.set_xticklabels(xt, fontsize=8)
    ax.set_ylabel("type-hint rate above baseline (pp)", fontsize=10)
    ...
    ax.set_title("Coupling prompts: eval-cue-attributable type-hinting,\nsplit into verbalized (VEA) vs latent channels",
                 fontsize=11, weight="bold")
    fig.text(0.5, 0.015, f"natural CoT · above own no-prefix baseline (Base {bvals['BASE']:.0f}%, Post-RL {bvals['r260']:.0f}%) "
                         f"· {lbl} × 60 · 95% CI", ha="center", fontsize=7, color="#555")
    fig.tight_layout(rect=(0, 0.05, 1, 1))
    out = os.path.join(OUT, f"coupling_decomp_{tag}.png")
    fig.savefig(out, dpi=140); plt.close(fig)
    print("wrote", out)
    for cname, _ in CONDS:
        for m, _ in MODELS:
            th, ve = (wood_cell(m, sub) if cname == "eval_wood" else couple_cell(m, cname, sub))
            thb = baseline(m, sub); B, ver, sil = decompose(th, ve, thb)
            print(f"   [{tag}] {cname:12s} {m:5s}: baseline {B:5.1f}%  VEA {ver:+6.1f}  Latent {sil:+6.1f}  TOTAL {ver+sil:+6.1f}")
